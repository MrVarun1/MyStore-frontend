...
const sortOptions = ["store_name", "email", "rating"];
...
const Stores = () => {
  ...
  useEffect(() => {
    fetchStoreList(sortBy);
  }, [sortBy]);

  const fetchStoreList = async (sort = "store_name", search = "") => {
    try {
      setStatus(statusConstants.LOADING);

      const response = await fetch(
        `${API_URL}/admin/stores?search=${search}&sort=${sort}`,
        {
          method: "GET",
          headers: {
          ...
          <div className="filter-container">
            ...
            <div className="filter">
              ...
              <Button
                type="button"
                onClick={() => {
                  fetchStoreList(sortBy, searchValue);
                }}
              >
                Search
              </Button>
